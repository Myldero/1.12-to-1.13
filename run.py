try:
	...
	import sys
	print("Run \"{} -m pip install nbt\" as administrator in terminal/cmd".format(sys.executable))
	input()
	exit()
...
try:
	nbtfile = NBTFile(os.path.join(worldpath, "level.dat"), "rb")
	...
	for file in files:
		if file.endswith(".mcfunction"):
			...
			with open(fullpath, 'r') as f:
				...
				for line in f:
					linenumber += 1
					filename = "{} line {}".format(fullpath[len(os.path.join(worldpath, "datapacks", datapack, "data")) + 1:-len(".mcfunction")], linenumber)
					memory += convert(line.rstrip())
...
after_time = time()
print("Done in {} seconds".format(round(after_time - before_time, 3)))
