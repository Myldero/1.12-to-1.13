...
displayName = "1.12 to 1.13"

inputs = (
    ("Updates command and structure blocks to 1.13", "label"),
    ("Datapack name (Alphanumeric) ", ("string","value=")),
    ("Check the console for error messages", "label"),
)
        ...
        for e in chunk.TileEntities:
            ...
            if (x,y,z) in box:
                if e["id"].value in ["minecraft:command_block","minecraft:chain_command_block","minecraft:repeating_command_block"]:
                    ...
                    e["Command"] = TAG_String(convert_command(e["Command"].value, "{} {} {}".format(x, y, z)))
                    chunk.dirty = True
                    
                elif e["id"].value == "minecraft:structure_block":
                    e["name"] = TAG_String( re.sub(r'([A-Za-z_/]+)', r'{}:structures/\1'.format(datapack), e["name"].value) )
                    chunk.dirty = True
